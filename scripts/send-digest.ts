...
async function sendViaMailgun(subject: string, text: string, html: string) {
  ...
  if (configuredValues.length === 0) {
    console.log("Mailgun not configured; digest was printed to stdout only.");
    return;
  }
  ...
  console.log(`Digest email sent to ${requiredDigestTo}.`);
}
...
async function main() {
  ...
  await sendViaMailgun(digest.subject, digest.text, digest.html);
}
